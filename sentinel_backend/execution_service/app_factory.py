"""
Factory pattern for Execution Service to enable dependency injection and testing.
"""
...
from sentinel_backend.config.settings import (
    get_settings, get_service_settings, get_application_settings, get_network_settings
)
...
class ExecutionServiceConfig:
    """Configuration for Execution Service dependencies."""
    
    def __init__(
        self,
        data_service_url: Optional[str] = None,
        service_timeout: int = 60,
        test_execution_timeout: int = 30,
        mock_mode: bool = False,
        mock_http_client: Optional[Any] = None
    ):
        self.data_service_url = data_service_url
        self.service_timeout = service_timeout
        self.test_execution_timeout = test_execution_timeout
        self.mock_mode = mock_mode
        self.mock_http_client = mock_http_client
        
        # Get default settings if not provided
        if not data_service_url and not mock_mode:
            service_settings = get_service_settings()
            self.data_service_url = service_settings.data_service_url
            self.service_timeout = service_settings.service_timeout
        
        if not test_execution_timeout and not mock_mode:
            app_settings = get_application_settings()
            self.test_execution_timeout = app_settings.test_execution_timeout
...
def create_execution_app(config: Optional[ExecutionServiceConfig] = None) -> FastAPI:
    """
    Create a FastAPI app for the Execution Service with configurable dependencies.
    ...
    async def create_test_run(request: Request, suite_id: int, target_environment: str) -> Optional[int]:
        ...
        if config.mock_mode:
            return 1  # Return mock run ID
        
        correlation_id = structlog.contextvars.get_contextvars().get("correlation_id")
        headers = {"X-Correlation-ID": correlation_id} if correlation_id else {}
        
        try:
            client = config.mock_http_client if config.mock_http_client else httpx.AsyncClient()
            try:
                run_data = {
                    "suite_id": suite_id,
                    "status": "running",
                ...
                response = await client.post(
                    f"{config.data_service_url}/api/v1/test-runs",
                    json=run_data,
                ...
                if not config.mock_http_client:
                    await client.aclose()
    ...
    async def fetch_test_cases(request: Request, suite_id: int) -> List[Dict[str, Any]]:
        ...
        try:
            client = config.mock_http_client if config.mock_http_client else httpx.AsyncClient(timeout=config.service_timeout)
            try:
                response = await client.get(
                    f"{config.data_service_url}/api/v1/test-suites/{suite_id}/cases",
                    headers=headers
                )
    ...
    async def execute_test_cases(
        ...
        client = config.mock_http_client if config.mock_http_client else httpx.AsyncClient(timeout=config.test_execution_timeout)
        try:
        ...
        try:
            ...
            try:
                for result in results:
                    ...
                    response = await client.post(
                        f"{config.data_service_url}/api/v1/test-results",
                        json=result_data,
                        headers=headers
                    )
        ...
        try:
            ...
            try:
                ...
                response = await client.patch(
                    f"{config.data_service_url}/api/v1/test-runs/{run_id}",
                    json=update_data,
                    headers=headers
                )
    ...
    async def get_test_run_status(request: Request, run_id: int):
        ...
        try:
            ...
            try:
                # Get run details
                run_response = await client.get(f"{config.data_service_url}/api/v1/test-runs/{run_id}", headers=headers)
                if run_response.status_code != 200:
                    raise HTTPException(status_code=404, detail="Test run not found")
                
                # Get run results
                results_response = await client.get(f"{config.data_service_url}/api/v1/test-runs/{run_id}/results", headers=headers)
